...
if KERNEL == 'arccosine':
    kern = [cov.ArcCosine(data.X.shape[1], degree=DEGREE, depth=DEPTH,
                          lengthscale=LENGTHSCALE, std_dev=1.0, input_scaling=IS_ARD)
            for i in range(1)]
else:
    kern = [cov.SquaredExponential(data.X.shape[1], length_scale=LENGTHSCALE, input_scaling=IS_ARD) for i in range(1)]
